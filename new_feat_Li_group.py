...
#this file for testing on grouped data
...
def main(finetune_config):
    ...
    if finetune_config['CV_flag']:
        ...
        dataset['chain architecture'] = dataset['chain architecture'].replace({'S_1': 0, 'S_2': 1})
        unique_polymers = dataset['Polymer'].unique()
        unique_anions = dataset['Anion Smiles'].unique()

        j=1
        groups = np.zeros(len(dataset['Anion Smiles']))
        lookup = {}
        ...
        for ind in dataset.index:
            groups[ind] = lookup[(dataset['Polymer'][ind],dataset['Anion Smiles'][ind])]
...
if __name__ == "__main__":
    ...
    finetune_config = yaml.load(
        open("config_new_feat_group.yaml", "r"), Loader=yaml.FullLoader)
