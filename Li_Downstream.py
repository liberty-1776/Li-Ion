...
from Li_dataset import Downstream_Dataset, DataAugmentation
...
class DownstreamRegression(nn.Module):
    ...
    def forward(self, input_ids1, attention_mask1, anion_smile_encoding, log_li, comonomer_percentage, approxMW, approxTg, chain_architecture):
        outputs = self.PretrainedModel(input_ids=input_ids1, attention_mask=attention_mask1)
        logits = outputs.last_hidden_state[:, 0, :]
        smile_embeddings = self.smile_autoencoder(logits)
        input_to_regressor = torch.cat([smile_embeddings, anion_smile_encoding, log_li.unsqueeze(1), comonomer_percentage.unsqueeze(1), approxMW.unsqueeze(1), approxTg.unsqueeze(1), chain_architecture.unsqueeze(1)], dim=1).clone().detach()
...
def train(model, optimizer, scheduler, loss_fn, train_dataloader, device):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        outputs = model(input_ids, attention_mask, anion_smile_encoding, log_li,
                        comonomer_percentage, approxMW, approxTg, chain_architecture).float()
...
def main(finetune_config):
    ...
    if finetune_config['CV_flag']:
        ...
        for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(data.shape[0]))):
            ...
            train_data[['approxMW(kDa)', 'approxTg']] = scaler_feat.fit_transform(
                train_data[['approxMW(kDa)', 'approxTg']])
            test_data[['approxMW(kDa)', 'approxTg']] = scaler_feat.transform(
                test_data[['approxMW(kDa)', 'approxTg']])
...
if __name__ == "__main__":
    ...
    finetune_config = yaml.load(
        open("config_Li_Ion.yaml", "r"), Loader=yaml.FullLoader)
